...
abstract class Game extends JFrame implements Runnable, KeyListener, MouseListener, MouseMotionListener {
	...
	public void loadData(String fileName) {
		...
		Object obj = null;
		try {
			file_in = new FileInputStream("saves/" + fileName);
	 ...
	 *******************************************************************************/
	protected void updateData(Map currentMap, Map currentOverlay, int playerX, int playerY, int mapX, int mapY, int health,
			int frisbee, int gfrisbee, int files, int snack, int keyboard) {
		data.update(currentMap.mapName(), currentOverlay.mapName(), playerX, playerY, mapX, mapY, health, frisbee, gfrisbee,
				files, snack, keyboard);
	}
